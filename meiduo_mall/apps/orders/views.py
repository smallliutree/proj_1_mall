...
class CommentView(LoginRequiredMixin, View):
    def post(self, request):
        ...
        if not all([address_id, pay_method]):
            return JsonResponse({'code':400,'errmsg':'参数错误1'})

        try:
            address = Address.objects.get(id=address_id)
        except Address.DoesNotExist:
            return JsonResponse({'code': 400, 'errmsg': '参数错误2'})

        if pay_method not in [1, 2]:
            return JsonResponse({'code': 400, 'errmsg': '参数错误3'})
        ...
        with transaction.atomic():
            strat_transaction = transaction.savepoint()
            ...
            for id in ids:
                ...
                if custom_count > sku.stock:
                    transaction.savepoint_rollback(strat_transaction)
            ...
            order.save()
            transaction.savepoint_commit(strat_transaction)
